...
console.log("KMap Extension: Content Script Loaded"); // Dòng này để biết script có chạy hay không

function injectButtons() {
    const containers = document.querySelectorAll('div[data-testid*="conversation-turn"]');
    console.log(`KMap Extension: Found ${containers.length} containers`); // Xem có tìm thấy container không

    containers.forEach(container => {
        if (container.querySelector('.kmap-add-btn')) return;
        
        // Tìm thanh công cụ của ChatGPT (nơi có nút Copy, Like)
        const toolbar = container.querySelector('.flex.justify-between') || container.querySelector('.mt-1');
        
        if (toolbar) {
            const btn = document.createElement('button');
            btn.className = 'kmap-add-btn';
            btn.innerText = '📍 Add to Map';
            toolbar.appendChild(btn);
            console.log("KMap Extension: Button injected!");
        }
    });
}
...
// Theo dõi thay đổi khi AI đang trả lời
